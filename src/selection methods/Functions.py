# -*- coding: utf-8 -*-
"""Untitled0.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1DjkJqapOyNMZ72LHmO9Fq-EnbscLYlSL
"""

import numpy as np
 
import torchvision.transforms as transforms
from torchvision.utils import make_grid

from torchvision import datasets
from torch.autograd import Variable
 
import torch.nn as nn
import torch.nn.functional as F
import torch
 
import os
import glob
 
import matplotlib.pyplot as plt
from tqdm.notebook import tqdm
import warnings
from tensorflow import keras,summary
import os
from tqdm import tqdm

path = '/content/drive/MyDrive/gan-getting-started/monet_jpg'
#path = 'sample_data/Monet'

def Load_images():
  images = []
  for file in tqdm(os.listdir(path)):
      images.append(plt.imread(path+'/'+file))
  images = np.array(images)
  return images




"""FID -calculating the frechet inception distance"""

import numpy
from numpy import cov
from numpy import trace
from numpy import iscomplexobj
from numpy import asarray
from numpy.random import randint
from scipy.linalg import sqrtm
from keras.applications.inception_v3 import InceptionV3
from keras.applications.inception_v3 import preprocess_input


#inputs:
#act1,act2 =The features extractions of 2 groups of images
#the features extractions done by trained model:InceptionV3/VGG..
#output:
#FID- calculated frechet inception distance
def calculate_fid( act1, act2):

	# calculate mean and covariance statistics
	mu1, sigma1 = act1.mean(axis=0), cov(act1, rowvar=False)
	mu2, sigma2 = act2.mean(axis=0), cov(act2, rowvar=False)
	# calculate sum squared difference between means
	ssdiff = numpy.sum((mu1 - mu2)**2.0)
	# calculate sqrt of product between cov
	covmean = sqrtm(sigma1.dot(sigma2))
	# check and correct imaginary numbers from sqrt
	if iscomplexobj(covmean):
		covmean = covmean.real
	# calculate score
	fid = ssdiff + trace(sigma1 + sigma2 - 2.0 * covmean)
	return fid



#rand 100 vectors of 30 indexs and calculate the FID to them
#return the min FID, and the Indexes whith the min FID
def Find_indexes_whith_min_Fid_from_random_vectors(act1,numRand=100):
	min=1000
	minIndx=np.zeros(30)
	for number in range(numRand):   
		rndArr=np.random.choice(300, 30)
		act2 = act1[rndArr,]
		#selectImg1 = preprocess_input(selectImg)
		fid = calculate_fid( act1, act2)
		if fid < min:
			print('FID (temp min): %.3f' % fid)
			print(number)
			min=fid
			minIndx= rndArr
			print(minIndx)		
	print('MIN Final---FID : %.3f' % min)
	print('The Selected Indexes Are: ')
	print(minIndx)

	return min, minIndx